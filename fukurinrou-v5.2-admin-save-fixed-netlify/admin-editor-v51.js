...
function imageFromFile(file,cb){
 const img=new Image(),r=new FileReader();r.onload=()=>{img.onload=()=>{const max=1000,scale=Math.min(1,max/Math.max(img.width,img.height)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));c.getContext('2d').drawImage(img,0,0,c.width,c.height);cb(c.toDataURL('image/jpeg',.82))};img.src=String(r.result)};r.readAsDataURL(file)
}
function render(){
...
function renderCatList(list){
 list.innerHTML='';draft.forEach((sec,i)=>{const b=document.createElement('button');b.className=`v51-cat-btn ${i===active?'active':''}`;b.draggable=true;b.dataset.i=i;b.innerHTML=`<span class="v51-drag">☰</span><span><span class="v51-cat-label">${esc(sec.cat)}</span><span class="v51-cat-count"> ${(sec.items||[]).length}品</span></span><span class="v51-cat-arrows"><span class="v51-mini" data-up>↑</span><span class="v51-mini" data-down>↓</span></span>`;b.onclick=e=>{if(e.target.closest('[data-up]')){e.preventDefault();move(draft,i,i-1);active=Math.max(0,i-1);render();return}if(e.target.closest('[data-down]')){e.preventDefault();move(draft,i,i+1);active=Math.min(draft.length-1,i+1);render();return}active=i;expanded='';render()};b.ondragstart=e=>{b.classList.add('dragging');e.dataTransfer.setData('text/plain',String(i))};b.ondragend=()=>b.classList.remove('dragging');b.ondragover=e=>e.preventDefault();b.ondrop=e=>{e.preventDefault();const from=Number(e.dataTransfer.getData('text/plain')),to=i;move(draft,from,to);active=to;render()};list.appendChild(b)})
}
function renderItems(box,sec){
 box.innerHTML='';(sec.items||[]).forEach((it,ii)=>{
   const row=document.createElement('div');row.className=`v51-item ${expanded===it.id?'expanded':''}`;row.draggable=true;row.dataset.ii=ii;
   ...
   row.ondragstart=e=>{if(e.target.closest('input,textarea,select,button,label')){e.preventDefault();return}row.classList.add('dragging');e.dataTransfer.setData('text/plain',String(ii))};row.ondragend=()=>row.classList.remove('dragging');row.ondragover=e=>e.preventDefault();row.ondrop=e=>{e.preventDefault();const from=Number(e.dataTransfer.getData('text/plain'));move(sec.items,from,ii);render()};
...
async function save(){
 const btn=document.querySelector('.v51-save'),st=document.querySelector('.v51-status');if(btn)btn.disabled=true;if(st)st.textContent='保存中…';
 try{normalizeSoups(draft);await firebase.database().ref('menu').set(draft);dirty=false;if(st)st.textContent='保存しました';setTimeout(()=>{if(btn)btn.disabled=false},500)}catch(e){console.error(e);if(st)st.textContent='保存に失敗しました';if(btn)btn.disabled=false}
}
